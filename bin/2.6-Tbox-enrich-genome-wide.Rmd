...
Scan all the promoters regions for the T-box motifs. Use as background the same sequences.
```{r genome-wide-scan}
useBigMemoryPWMEnrich(TRUE) # using large amount of memory
gw = motifEnrichment(prs, bg.custom)
useBigMemoryPWMEnrich(FALSE) # disable using large amount of memory
```

Save the scans for later retrieval.
```{r save-gw-scan}
save(gw, file = paste0(dir,"data/pwmEnrich-logn-genomoeWide.Rdata"))
```

```{r load-gw-scan}
load(paste0(dir,"data/pwmEnrich-logn-genomoeWide.Rdata"))
```


Check the gene overlaps when different *p*-value thresholds are set. No need to check TBX-33, as it is not the focus for the rest of the manuscript.
```{r coverage-thresholds}
p.05 <- gw$sequence.bg[,2:5] < .05
p.01 <- gw$sequence.bg[,2:5] < .01
p.001 <- gw$sequence.bg[,2:5] < .001
p.0001 <- gw$sequence.bg[,2:5] < .0001

par(mfrow=c(2,2))
plot(venn(p.05),main="p-value < .05")
plot(venn(p.01),main="p-value < .01")
plot(venn(p.001),main="p-value < .001")
plot(venn(p.0001),main="p-value < .0001")
```
From this I conclude that `.001` and `.0001` are way too low. Both `.05` and `.01` show the overlaps well. As the next figure shows the *anc-1* RNAi regulated genes that have  T-box binding motifs with a *p*<0.01, for consistency, I'll display the `.01`-threshlod passing motifs.

```{r focus-p<0.01}
p.01 <- gw$sequence.bg[,2:5] < .01
colnames(p.01) <- c("TBX-38","TBX-39","TBX-40","TBX-43")
```

```{r euler-plot}
vd <- euler(p.01[,c(1,4)], shape = "ellipse") # Generate Euler
