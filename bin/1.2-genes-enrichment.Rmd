---
title: "Pathway and Annotation Enrichment Analysis"
author: "Amir Levine"
date: "2018-11-06"
output: md_document
---
...
2. `BOTH` is a tibble that contains the RNA-seq gene expression information and statistics for the two strain.
```{r}
load("data/geneExpression.rdata")
```

# Define the Genes of interest for the following analyses
A vector of the Gene IDs for all the *anc-1* RNAi regulated genes.
Narrow down the gene expression datarame to goi
```{r}
goi <- DEG$UNION.reg
```
...
# GO Gene Set Enrichment Analysis (GSEA) - *fgsea*
Perform GSEA using the package *fgsea*.

Fetch BioMart object.
```{r}
mart = useMart(biomart="ensembl", dataset="celegans_gene_ensembl")
```

Go over possible annotations
```{r}
...
```{r}
att <- c("ensembl_gene_id","go_id","name_1006","definition_1006","go_linkage_type","namespace_1003")
go_annotation <- getBM(attributes = att, 
                       filters=list(biotype='protein_coding'),
                       mart=mart)

att <- c("ensembl_gene_id","goslim_goa_accession","name_1006","goslim_goa_description","namespace_1003")
goslim_annotation <- getBM(attributes = att, 
                       filters=list(biotype='protein_coding'),
                       mart=mart)

save(go_annotation,goslim_annotation, file = "data/2018-11-06-GO-annotations.Rdata")
```
...
goslim_annotation_bp <- goslim_annotation[which(goslim_annotation$namespace_1003 == "biological_process"),]
```

Generate **Gene-Sets**, a list object in which there are vectors of Ensembl Gene IDs, each labelled with a Go term.
```{r}


go_pathway_sets <- split(go_annotation_bp$ensembl_gene_id,
                         paste(go_annotation_bp$goslim_goa_accession,
                               go_annotation_bp$goslim_goa_description))
...
The list of genes needs to be sorted for GSEA analysis. It is sorted based on the FDR-adjusted *p*-value (*p*) and the gene expression fold-change (*FC*) between treatment (*anc-1* RNAi) and the control. The sorting score is equal to $-log10(p)*log2(FC)$
The list is sorted in a descending order. Need to consider using just the *p*-values.
```{r}
geneList <- -log10(NT$padj) * NT$log2FoldChange
names(geneList) <- NT$Gene_stable_ID
